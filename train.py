...
parser.add_argument("--case_mapping_file", type=str, required=True)
parser.add_argument('--batch_size', type=int, default=16)
parser.add_argument('--mode', type=str, default='train', choices=['train', 'test'])
parser.add_argument('--model', type=str, default='raunet')
parser.add_argument('--task', type=str, default='tumor', choices=['tumor','kidney'])
...
def main():
    ...
    logger = TensorBoardLogger(
        'log',
        name='%s_%s_%s' % (args.dataset, args.task, args.model)
    )
...
def test():
    args = parser.parse_args()
    ckpt = os.path.join(args.checkpoint_path, 'last.ckpt')
    model = SegmentationPLModel.load_from_checkpoint(ckpt)
